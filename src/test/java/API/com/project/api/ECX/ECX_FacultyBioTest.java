...
public class ECX_FacultyBioTest extends BaseTest{
	...
	public void setUp()
	{
		parser.setAPIJsonFile("regFaculty");
	}
	...
	{
		try{
		  	Reporter.startTest("FacultyBio ", "Check HTTP Status code is 200");

			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.FACULTY_BIO.toString(),false);	
	...
	{
		try{
		  	Reporter.startTest("FacultyBio ", "Check All headers are present");
	...
	{
		try{
		  	Reporter.startTest("FacultyBio ", "Check that API response returns record");
	...
	{
		try{
			Reporter.startTest("FacultyBio ", "Check that API response returns * records when min is set to 0 and post fix to 8");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.FACULTY_BIO.toString(),false)+
	...
	T5_test_Check_A_SpecificRecord() throws Exception 	{
		try {
			...
			Reporter.startTest("FacultyBio ", "Validates API response for a specific record");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.FACULTY_BIO.toString(),false);	
		    assured.getRequestWithAuthorization(getProgramsEndPoint);
			this.res = assured.returnLastResponse();
	...
	{
		try{
		  	Reporter.startTest("FacultyBio ", "Validates API response for records in range");
	...
	{
		try{
		    Reporter.startTest("FacultyBio ", "Check that no of recorda re same if both Offset +"
	...
	{
		try{
			Reporter.startTest("FacultyBio ", "Check that no of records if Offset is set to 0");
		    String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.FACULTY_BIO.toString() 
		    		,false) +"??offset=0";
