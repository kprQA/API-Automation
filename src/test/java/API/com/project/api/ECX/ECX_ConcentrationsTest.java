...
public class ECX_ConcentrationsTest extends BaseTest{
	...
	public void setUp()
	{
		parser.setAPIJsonFile("regConcentrations");
	}
	...
	{
		try{
		  	Reporter.startTest("Concentrations ", "Check HTTP Status code is 200");

			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.CONCENTRATIONS.toString(),false);	
	...
	{
		try{
		  	Reporter.startTest("Concentrations ", "Check All headers are present");
	...
	{
		try{
		  	Reporter.startTest("Concentrations ", "Check that API response returns record");
	...
	{
		try{
			Reporter.startTest("Concentrations ", "Check that API response returns * records when min is set to 0 and post fix to 8");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.CONCENTRATIONS.toString(),false)+
	...
	T5_test_Check_A_SpecificRecord() throws Exception
	{
		try {
			...
			Reporter.startTest("Concentrations ", "Validates API response for a specific record");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.CONCENTRATIONS.toString(),false);	
		    assured.getRequest(getProgramsEndPoint);
			this.res = assured.returnLastResponse();
	...
	{
		try{
		  	Reporter.startTest("Concentrations ", "Validates API response for records in range");
	...
	{
		try{
		    Reporter.startTest("Concentrations ", "Check that no of recorda re same if both Offset +"
	...
	{
		try{
			Reporter.startTest("Concentrations ", "Check that no of records if Offset is set to 0");
		    String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.CONCENTRATIONS.toString() 
		    		,false) +"??offset=0";
