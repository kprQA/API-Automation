...
public class ECX_CoursesTest extends BaseTest{
	...
	public void setUp()
	{
		parser.setAPIJsonFile("regCourse");
	}
	...
	{
		try{
		  	Reporter.startTest("Courses ", "Check HTTP Status code is 200");

			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.COURSES.toString(),false);	
	...
	{
		try{
		  	Reporter.startTest("Courses ", "Check All headers are present");
	...
	{
		try{
		  	Reporter.startTest("Courses ", "Check that API response returns record");
	...
	{
		try{
			Reporter.startTest("Courses ", "Check that API response returns * records when min is set to 0 and post fix to 8");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.COURSES.toString(),false)+
	...
	T5_test_Check_A_SpecificRecord() throws Exception
	{
		try {
			...
			Reporter.startTest("Courses ", "Validates API response for a specific record");
			String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.COURSES.toString(),false);	
		    assured.getRequest(getProgramsEndPoint);
			this.res = assured.returnLastResponse();
	...
	{
		try{
		  	Reporter.startTest("Courses ", "Validates API response for records in range");
	...
	{
		try{
		    Reporter.startTest("Courses ", "Check that no of recorda re same if both Offset +"
	...
	{
		try{
			Reporter.startTest("Courses ", "Check that no of records if Offset is set to 0");
		    String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.COURSES.toString() 
		    		,false) +"??offset=0";
