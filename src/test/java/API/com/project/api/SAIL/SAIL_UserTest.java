...
import io.restassured.response.Response;
	...
	public class SAIL_UserTest extends BaseTest {
		...
		{
			try{
			  	Reporter.startTest("SAIL_USER", "Check HTTP Status code is 200");

				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_USER.toString(),false);	
		...
		{
			try{
			  	Reporter.startTest("SAIL_USER", "Check All headers are present");
		...
		{
			try{
			  	Reporter.startTest("SAIL_USER", "Check that API response returns record");
		...
		{
			try{
				Reporter.startTest("SAIL_USER", "Check that API response returns * records when min is set to 0 and post fix to 2");
				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_USER.toString(),false)+
		...
		T5_test_Check_A_SpecificRecord() throws Exception
		{
			try {
				...
				Reporter.startTest("SAIL_USER", "Validates API response for a specific record");
				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_USER.toString(),false);	
			    assured.getRequestWithAuthorization(getProgramsEndPoint);
				this.res = assured.returnLastResponse();
		...
		{
			try{
			  	Reporter.startTest("SAIL_USER", "Validates API response for records in range");
		...
		{
			try{
			    Reporter.startTest("SAIL_USER", "Check that no of recorda re same if both Offset +"
		...
		{
			try{
				Reporter.startTest("SAIL_USER", "Check that no of records if Offset is set to 0");
			    String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_USER.toString() 
			    		,false) +"??offset=0";
