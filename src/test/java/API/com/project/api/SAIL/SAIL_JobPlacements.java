...
import io.restassured.response.Response;
	...
	public class SAIL_JobPlacements extends BaseTest {
		...
		public void setUp()
		{
			parser.setAPIJsonFile("regJobPlacements");
		}
		...
		{
			try{
			  	Reporter.startTest("SAIL_JOBPLACEMENTS", "Check HTTP Status code is 200");

				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_JOBPLACEMENTS.toString(),false);	
		...
		{
			try{
			  	Reporter.startTest("SAIL_JOBPLACEMENTS", "Check All headers are present");
		...
		{
			try{
			  	Reporter.startTest("SAIL_JOBPLACEMENTS", "Check that API response returns record");
		...
		@Test() public void
		T4_test_OffSet_And_Max_When_Set_To_Some_Value_Records_Are_Displayed_While_Using_Pagesize() throws Exception
		{
			try{
				Reporter.startTest("SAIL_JOBPLACEMENTS", "Check that API response returns * records when min is set to 0 and post fix to 1");
				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_JOBPLACEMENTS.toString(),false)+
		...
		T5_test_Check_A_SpecificRecord() throws Exception
		{
			try {
				...
				Reporter.startTest("SAIL_JOBPLACEMENTS", "Validates API response for a specific record");
				String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_JOBPLACEMENTS.toString(),false);	
			    assured.getRequestWithAuthorization(getProgramsEndPoint);
				this.res = assured.returnLastResponse();
		...
		{
			try{
			  	Reporter.startTest("SAIL_JOBPLACEMENTS", "Validates API response for records in range");
		...
		{
			try{
			    Reporter.startTest("SAIL_JOBPLACEMENTS", "Check that no of recorda re same if both Offset +"
					...
					getProgramsEndPoint=getProgramsEndPoint.replace("minValue", "2");
					getProgramsEndPoint=getProgramsEndPoint.replace("maxValue", "2");
				    assured.getRequestWithAuthorization(getProgramsEndPoint);
					this.res = assured.returnLastResponse();
					assured.assertStatusCodeForResponse(res,endPoint.HTTP_STATUS_OK);
		...
		{
			try{
				Reporter.startTest("SAIL_JOBPLACEMENTS", "Check that no of records if Offset is set to 0");
			    String getProgramsEndPoint = endPoint.returnEndPoint(APIEndPoints.SAIL_JOBPLACEMENTS.toString() 
			    		,false) +"??offset=0";
				...
				Assert.assertEquals(valForMaxSize, "3", "Max size header value didn't match"
